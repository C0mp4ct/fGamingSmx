...
public CheckClientBan(const client) {
    ...
    Format(DBQuery, sizeof(DBQuery), " \
        SELECT b.bantime, b.length, b.reason, pa.name, b.date \
        FROM banlist AS b \
        JOIN players p ON b.idPlayer=p.id \
        LEFT JOIN players pa ON b.idAdmin=pa.id \
        WHERE b.idPlayer='%d' \
    ", g_PlayerId[client]);
